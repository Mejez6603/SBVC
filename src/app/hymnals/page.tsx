...
import { X, Play, RefreshCw, Maximize } from 'lucide-react';
...
export default function HymnalsPage() {
  ...
  const handleContentSelectForPreview = (content: string[], reference: string, index: number) => {
    ...
    if (index === -1) { // Title is selected
        newPassage = {
            reference: reference,
            text: ''
        };
    } else { // A verse is selected
        newPassage = {
            reference: '',
            text: content.join('\n')
        };
